...
public extension NavigationGraph {
    ...
    func present(segue: Segue<N>) throws {
        ...
        guard navFlow.has(segue: segue) else {
        ...
        for trait in traits {
            switch trait {
            case let .redirect(redirectFlow):
        ...
        if !traits.contains(.cover), !traits.contains(.modal) {
        ...
        for segue in navFlow.egressSegues(for: segue.out) {
    ...
    func forward() throws {
        ...
        if let last = pathFlow.segues.last?.out {
            ...
            let entries = Set(navFlow.inlets.map { $0.in })
        ...
        segues = navFlow.egressSegues(for: startNode)
    ...
    func dismiss(context: Bool = false) throws {
    ...
    func dismiss(segue: Segue<N>) throws {
        guard navFlow.has(segue: segue.counterpart) else {
